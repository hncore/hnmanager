 ...
 *
 */
if ( ! class_exists( 'HCF_Field_dimensions' ) ) {
  class HCF_Field_dimensions extends HCF_Fields {
    ...
    public function render() {
      ...
      $args = wp_parse_args( $this->field, array(
        ...
        'width_placeholder'  => esc_html__( 'width', 'hcf' ),
        'height_placeholder' => esc_html__( 'height', 'hcf' ),
        'width'              => true,
        'height'             => true,
        'unit'               => true,
        'show_units'         => true,
      ...
      if ( ! empty( $args['width'] ) ) {
        ...
        echo ( ! empty( $args['width_icon'] ) ) ? '<span class="hcf--label hcf--icon">'. $args['width_icon'] .'</span>' : '';
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[width]' ) ) .'" value="'. esc_attr( $value['width'] ) .'"'. $placeholder .' class="hcf-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( ! empty( $unit ) ) ? '<span class="hcf--label hcf--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
      }
      ...
      if ( ! empty( $args['height'] ) ) {
        ...
        echo '<div class="hcf--input">';
        echo ( ! empty( $args['height_icon'] ) ) ? '<span class="hcf--label hcf--icon">'. $args['height_icon'] .'</span>' : '';
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[height]' ) ) .'" value="'. esc_attr( $value['height'] ) .'"'. $placeholder .' class="hcf-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( ! empty( $unit ) ) ? '<span class="hcf--label hcf--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
      }
